	...
	<view>
		...
		<view class="hello">
			...
			<view class="base">
				...
				<view class="item"> 
					字颜色：
					<button class="button" @click="open" :style="{backgroundColor:'rgba('+fontcolor.r+','+fontcolor.g+','+fontcolor.b+','+fontcolor.a+')'}"> </button>
					       <!-- 需要声明 ref  -->
				...
				<view class="item">
					...
					<button class="button" @click="open1" :style="{backgroundColor:'rgba('+biaotise.r+','+biaotise.g+','+biaotise.b+','+biaotise.a+')'}"> </button>
					       <!-- 需要声明 ref  -->
				...
				<view class="item">
					...
					<button class="button" @click="open2" :style="{backgroundColor:'rgba('+geshangse.r+','+geshangse.g+','+geshangse.b+','+geshangse.a+')'}"> </button>
					       <!-- 需要声明 ref  -->
				...
				<view class="item">
					...
					<button class="button" @click="open3" :style="{backgroundColor:'rgba('+gexiase.r+','+gexiase.g+','+gexiase.b+','+gexiase.a+')'}"> </button>
					       <!-- 需要声明 ref  -->
				...
				<view class="item">
					...
					<button class="button" @click="open4" :style="{backgroundColor:'rgba('+bgcolor.r+','+bgcolor.g+','+bgcolor.b+','+bgcolor.a+')'}"> </button>
					       <!-- 需要声明 ref  -->
			...
			<view class="content" id="content" :style="{background: 'rgba('+bgcolor.r+','+bgcolor.g+','+bgcolor.b+','+bgcolor.a+')'}" @click="changebgcolor">
							...
							<td v-for="(item1, idx1) in item" :key="idx1" :style="{width:kuan,height:gao}">
								<view class="xx" :style="{margin:waibianju,padding:neibianju,fontSize:fontsize,width:kuan,height:gao,lineHeight:gao}">
									<view class="d1" :style="{background:'rgba('+gexiase.r+','+gexiase.g+','+gexiase.b+','+gexiase.a+')'}">
										<view class="d2" :style="{background:'rgba('+geshangse.r+','+geshangse.g+','+geshangse.b+','+geshangse.a+')'}"></view>
										<view class="d3" :style="{color:'rgba('+fontcolor.r+','+fontcolor.g+','+fontcolor.b+','+fontcolor.a+')',fontSize:fontsize,}">{{item1}}</view>
									</view>
								</view>
							</td>
						</tr>
	...
	export default {
		...
		data() {
			return {
				...
				firstWord: uni.getStorageSync("firstWord1") || "广",
				secondWord: uni.getStorageSync("secondWord1") || "云",
				hang: uni.getStorageSync("hang1") || 10,
				lie: uni.getStorageSync("lie1") || 10,
				kuan: uni.getStorageSync("kuan1") || "20px",
				gao: uni.getStorageSync("gao1") || "20px",
				waibianju: uni.getStorageSync("waibianju1") || "3px",
				neibianju: uni.getStorageSync("neibianju1") || "3px",
				fontsize: uni.getStorageSync("fontsize1") || "12px",
				biaotigao: uni.getStorageSync("biaotigao1") || "40px",

				fontcolor:uni.getStorageSync("fontcolor1") ?JSON.parse(uni.getStorageSync("fontcolor1")) :{r: 255,g: 60,b: 90,a: 0.6},
				biaotise:uni.getStorageSync("biaotise1")? JSON.parse(uni.getStorageSync("biaotise1")): {r: 255,g: 100,b: 0,a: 0.6},
				geshangse:uni.getStorageSync("geshangse1")?JSON.parse(uni.getStorageSync("geshangse1")) : {r: 255,g: 0,b: 100,a: 0.6},
				gexiase:uni.getStorageSync("gexiase1")? JSON.parse(uni.getStorageSync("gexiase1")) : {r: 55,g: 100,b: 0,a: 0.6},
				bgcolor:uni.getStorageSync("bgcolor1") ? JSON.parse(uni.getStorageSync("bgcolor1")): {r: 155,g: 0,b: 0,a: 0.6},

				// fontcolor:{r: 255,g: 60,b: 90,a: 0.6},
				// biaotise: {r: 255,g: 100,b: 0,a: 0.6},
				// geshangse : {r: 255,g: 0,b: 100,a: 0.6},
				// gexiase : {r: 55,g: 100,b: 0,a: 0.6},
				// bgcolor: {r: 155,g: 0,b: 0,a: 0.6},
				
				biaoti: uni.getStorageSync("biaoti1") || "从1中找2",
				table_data: [],
				table_data1: [],
		...
		// created() {
		// 	// 获取缓存中的颜色
		// 	var color1 = uni.getStorageSync("fontcolor1");
		// 	if(color1){
		// 		this.fontcolor = JSON.parse(uni.getStorageSync("fontcolor1"))
		// 	}
		// },
		watch: {
			// 将数据保存下来
			firstWord() {
				uni.setStorageSync("firstWord1", this.firstWord);
			},
			biaoti() {
				uni.setStorageSync("biaoti1", this.biaoti);
			},
			fontsize() {
				uni.setStorageSync("fontsize1", this.fontsize);
			},
			biaotigao() {
				uni.setStorageSync("biaotigao1", this.biaotigao);
			},
			neibianju() {
				uni.setStorageSync("neibianju1", this.neibianju);
			},
			waibianju() {
				uni.setStorageSync("waibianju1", this.waibianju);
			},
			secondWord() {
				uni.setStorageSync("secondWord1", this.secondWord);
			},
			hang() {
				uni.setStorageSync("hang1", this.hang);
			},
			lie() {
				uni.setStorageSync("lie1", this.lie);
			},
			kuan() {
				uni.setStorageSync("kuan1", this.kuan);
			},
			gao() {
				uni.setStorageSync("gao1", this.gao);
			},
			// 颜色
			geshangse() {
				uni.setStorageSync("geshangse1",JSON.stringify(this.geshangse) );
			},
			gexiase() {
				uni.setStorageSync("gexiase1", JSON.stringify(this.gexiase));
			},
			fontcolor() {
				uni.setStorageSync("fontcolor1", JSON.stringify(this.fontcolor));
			},
			biaotise() {
				uni.setStorageSync("biaotise1", JSON.stringify(this.biaotise));
			},
			bgcolor() {
				uni.setStorageSync("bgcolor1", JSON.stringify(this.bgcolor));
			},
		},
		methods: {
			// 字的颜色
			...
			btn1click() {
				...
				this.biaoti = '从 "' + first + '" 中找到 "' + second + '"';
				console.log("this.biaoti", this.biaoti)
				// 根据输入的行和列生成数据
				for (var i = 0; i < this.hang; i++) {
					var data = []
